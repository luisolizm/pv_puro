...
<style>
  ...
  /* ── Ocultar texto "keyboard_dc..." del botón colapsar sidebar ── */
...
def calc_ppa_result(gen1: float, inv_usd: float, precio_ppa: float,
    ...
    # FIX: Valor residual del sistema al final del contrato PPA
    # Si el contrato es más corto que la vida útil, el activo sigue generando valor.
    # Se estima como VPN de los flujos futuros post-contrato (solo ingresos netos de O&M,
    # sin precio de venta al cliente — se usa precio PPA escalado como proxy conservador).
    ...
    if anios_restantes > 0 and r > 0:
        ...
        # Flujo neto anual post-contrato (estimado conservador)
        fn_post   = gen_post * prec_post - om_post - seg_post
        # Factor de anuidad descontada para años restantes, traída a hoy
        factor_anualidad = (1 - (1 + r) ** (-anios_restantes)) / r
        valor_residual   = fn_post * factor_anualidad / (1 + r) ** plazo
    else:
        valor_residual = 0.0
    ...
    # FIX: payback con interpolación lineal
    pb = None
    acum_pb = -equity_mxn
...
""", unsafe_allow_html=True)
    ...
    with col_r:
        ...
        inversion     = inversion_usd  # mantener alias USD para compatibilidad con TOR/comparador
        ...
        with ex2:
            ...
            if st.button("💾 Guardar corrida en comparador", use_container_width=True):
                run = {
                    ...
                    "payback":   round(payback, 1),                    "vpn":       round(vpn, 0),
